...
int error=0;
//pt analiza tipurilor
int exi=0,o=0,ok;
int ct=0;
int tipv;//tipv- tipul pentru variabila
int oper1;
				
int loc1,loc2;
				//lval -1-valoare,2-adresa
...
int tipt,lvalt;//pentru Termen
int tipt1,lvalt1;//pentru Termen1
int tipt2,lvalt2;//pentru Termen2
int tipf,lvalf;//pentru Factor
int tipf1,lvalf1;//pentru Factor1
int tipf2,lvalf2;//pentru Factor2

int opmul;//1-*,2-/,3-div sau mod
int ltc,cont_par,tipp,trp,t1,t2,lltc;
char *num;
...
if(ope==1)//impartire
	...
	if (val2==0)
		{
			printf(" Eroare impartire cu 0 la linia: %d\n",q->linie);
			exit(1);
		}
	if((tip1==1)&&(tip2==1))
...
//pt analiza tipurilor
int Cauta_TS(char *nume)//cauta in tabsim de simboluri de jos in sus identificatorul cu numele specificat prin nume si intoarce pozitia/-1
{
	int i,ok=0;
	...
	while(i>=0)
	{
		if((strcmp(tabela[i].nume,nume)==0)&&((strcmp(tabela[i].clasa,"nume variabila simpla")==0) || (strcmp(tabela[i].clasa,"nume parametru formal valoare")==0) ||(strcmp(tabela[i].clasa,"nume parametru formal adresa")==0)||(strcmp(tabela[i].clasa,"nume constanta")==0)))
			return i;
		else
			i--;
	}
...
int Cauta_TS1(char *nume)//cauta in tabsim de simboluri de jos in sus identificatorul cu numele specificat prin nume si intoarce pozitia/-1
{//cauta pt record
	int i,ok=0,k=-1;
	...
	while((i>=0)&&(ok==0))
	{
		if((strcmp(tabela[i].nume,nume)==0)&&(strcmp(tabela[i].clasa,"nume camp structura")==0))
		{
			k=i;
			reco=tabela[k].listarec;
			while(reco!=NULL)
			{
				if (reco->poz==loc1)
					ok=1;
...
int Cauta_TS2(char *nume)//cauta in tabsim de simboluri de jos in sus identificatorul cu numele specificat prin nume si intoarce pozitia/-1
{//cauta pt tablou
	int i,ok=0;
	...
	while(i>=0)
	{
		if((strcmp(tabela[i].nume,nume)==0)&&(strcmp(tabela[i].clasa,"nume variabila tablou")==0))
			return i;
		else
			i--;
	}
...
int Cauta_TS3(char *nume)//cauta in tabsim de simboluri de jos in sus identificatorul cu numele specificat prin nume si intoarce pozitia/-1
{//cauta pt var struct
	int i,ok=0;
	...
	while(i>=0)
	{
		if((strcmp(tabela[i].nume,nume)==0)&&(strcmp(tabela[i].clasa,"nume variabila structura")==0))
			return i;
		else
			i--;
	}
...
int Cauta_TS4(char *nume)//cauta in tabsim de simboluri de jos in sus identificatorul cu numele specificat prin nume si intoarce pozitia/-1
{//cauta pt fctsau procedura
	int i,ok=0;

	i=nr-1;
	while(i>=0)
	{
		if((strcmp(tabela[i].nume,nume)==0)&&((strcmp(tabela[i].clasa,"nume functie")==0)||(strcmp(tabela[i].clasa,"nume procedura")==0)))
			return i;
		else
			i--;
	}
		...
		if (ct==3)
			{
				if(tipp!=tip)
				{
					printf("Eroare: expresia  trebuie sa fie de acelasi tip cu valoarea ramurii in instr case la linia : %d\n",q->linie);
					return 0;
				}
	...
	if (Variabila())
	{
		if ((q->cod_lexical==3)&&(q->atribut==14))//operatorul :=
		{
			...
			if (Expresie())
			{
				...
				tipe7=tipe;
				lvale7=lvale;
				if (tipv!=tipe7)//tipe_i tipul expresiei de mai sus
				{
					if ((tipv==3) || (tipe7==3) || (tipv==1))
					{
						printf("Eroare, nu se poate converti tipul expr: %d la tipul var: %d la linia:%d \n",tipe,tipv,q->linie);
						exit(1);
					}
				}
...
int Variabila1()
{
	...
	if ((q->cod_lexical==4)&&(q->atribut==1))//delimitatorul '.'
	{
		...
		if (loc1==-1)
		{
			printf("Identificatorul %s nu a fost declarat la linia: %d\n",num, q->linie);
			return 0;
		}
		...
		{
			//analiza tipurilor
			if (strcmp(tabela[loc1].clasa,"nume variabila structura")!=0)
			{
				printf("Eroare, identificatorul trebuie sa fie nume de structura la linia: %d\n",q->linie);
				return 0;
			}
			else
			{
				loc2=Cauta_TS1(q->nume);
				if (loc2==-1)
				{
					printf("Identificatorul %s nu a fost declarat la linia: %d\n",q->nume, q->linie);
					return 0;
				}
				if (strcmp(tabela[loc2].clasa,"nume camp structura")==0)
				{
					reco=tabela[loc2].listarec;
					while(reco!=NULL)
					{
						if (reco->poz==loc1)
							ok=1;
					...
					if (ok==0)
					{
						printf("Eroare, identificatorul trebuie sa fie camp al indentificatorului de dinainte la linia:%d\n",q->linie);
						return 0;
					}
					else
					{
						tipv=tabela[loc2].tip;
					}
				}
				else
				{
	...
	else
	{
		if ((q->cod_lexical==3)&&(q->atribut==6))//operatorul [
		{
			...
			if (Expresie())
			{
				...
				{
					//analiza tipurilor
					if (strcmp(tabela[loc1].clasa,"nume variabila tablou")!=0)
					{
						printf("Eroare, identificatorul trebuie sa fie nume de tablou la linia: %d \n",q->linie);
						return 0;
					}
					else
					{
						if (tipe8!=1)//tipul expresiei de mai sus
						{
							printf("Eroare, indicele trebuie sa fie de tip intreg la linia: %d\n",q->linie);
							return 0;
						}
						else
							tipv=tabela[loc1].tip;
					}
		...
		else 
		{
			...
			if (loc1!=-1)
			{
				if ((strcmp(tabela[loc1].clasa,"nume variabila simpla")==0)  || (strcmp(tabela[loc1].clasa,"nume parametru formal valoare")==0) ||(strcmp(tabela[loc1].clasa,"nume parametru formal adresa")==0))
				{
					tipv=tabela[loc1].tip;
				}
				else
				{
					printf("Eroare, acest simbol nu este l-value q:%s\n",q->nume);
					return 0;
				}
			}
			else if (loc2!=-1)	
			{ 
				if (strcmp(tabela[loc2].clasa,"nume functie")==0)
				{
					i=nr-1;
					while(strcmp(tabela[i].clasa,"nume functie")!=0)//(strcmp(tabsim[i].clasa,"nume program                 ")!=0) && (strcmp(tabsim[i].clasa,"nume procedura               ")!=0)//variabila poate fi doar nume de functie!?
						i--;
					if (loc2!=i)
					{
						printf("Eroare, nu se poate atribui decat numele functiei curente la linia: %d\n",q->linie);
						return 0;
					}	
					else
					{
...
int Expresie1()
{
	if ((q->cod_lexical==3)&&((q->atribut==0)||(q->atribut==1)))//operatorii:+ -
	{
		...
		{
			//analiza tipurilor
			tipe1=tipe;//asta de la mine: tipe1 este tipe cand intra prima data in expresie 1
			if ((tipe1==3) || (tipt==3))
			{
				printf("Eroare, tipuri incompatibile cu operatia la linia:%d \n",q->linie);
				return 0; 
			}
			else
			{
...
int Termen1()
{
	if (((q->cod_lexical==3)&&(q->atribut==2)) || ((q->cod_lexical==3)&&(q->atribut==3)) ||((q->cod_lexical==1)&&(q->atribut==5)) ||((q->cod_lexical==1)&&(q->atribut==14)))// operatorii: * / si cuvintele cheie: div si mod
	{
		...
		{
			//analiza tipurilor
			tipt1=tipt;//asta de la mine: tipt1 este tipt cand intra prima data in termen 1
			if (opmul==3)
				tipt=1;
			else
				if (opmul==2)
...
int Factor()
{
	char *num2;
		...
		{
			//analiza tiputilor
			//lltc locul unde alex a inserat constanta in tab_const(variabila globala)
			tipf=tab[lltc].tip;
			lvalf=1;//valoare
					...
					if ((q->cod_lexical==3)&&(q->atribut==6))//operatorul [
					{
						...
						if (Expresie())
						{
							...
							if ((q->cod_lexical==3)&&(q->atribut==7))//operatorul ]
							{
								...
								{
									tipe4=tipe;//de la mine
									if (tipe4!=1)//tipul expresiei de mai sus
									{
										printf("Eroare, indicele trebuie sa fie de tip intreg la linia: %d\n",q->linie);
										return 0;
									}
					...
					else
					{
						...
						else 
						{
							if ((q->cod_lexical==3)&&(q->atribut==4))//operatorul (
							{
								...
								if (strcmp(tabela[loc1].clasa,"nume functie")!=0)
								{
									printf("Eroare, identificatorul trebuie sa fie nume de functie la linia: %d\n",q->linie);
									return 0;
								}
								else
								{
								...
								q=getNextAtom(q);
								if (ListaExpresii(loc1))
								{
									if ((q->cod_lexical==3)&&(q->atribut==5))//operatorul )
									{
										//aici analiza
										if (cont_par!=tabela[loc1].nrpar)
										{
											printf("Eroare, numar de parametrii prea mic! la linia: %d\n",q->linie);
											return 0;
										}
							...
							else 
							{
								...
								if (loc1!=-1)
								{
									if ((strcmp(tabela[loc1].clasa,"nume constanta")==0))
									{
										ltc=tabela[loc1].val;
										tipf=tab[ltc].tip;
										lvalf=1;//valoare
									}
									else if ((strcmp(tabela[loc1].clasa,"nume variabila simpla")==0) ||(strcmp(tabela[loc1].clasa,"nume variabila tablou")==0) || (strcmp(tabela[loc1].clasa,"nume variabila structura")==0)  ||(strcmp(tabela[loc1].clasa,"nume parametru formal adresa")==0))
									{
										tipf=tabela[loc1].tip;
										lvalf=2;//adresa
									}
									else if ( (strcmp(tabela[loc1].clasa,"nume parametru formal valoare")==0))
									{
										tipf=tabela[loc1].tip;
										lvalf=1;//valoare
									}
									...
									{
										printf("Eroare, simbol nepermis intr-o expresie la linia: %d\n",q->linie);
										return 0;
									}
								}
								else if (loc2!=-1)
								{
									if (strcmp(tabela[loc2].clasa,"nume functie")==0)
									{
										if (tabela[loc2].nrpar!=0)
										{
											printf("Eroare, numar de parametrii eronat la apel la linia: %d\n",q->linie);
											return 0;
										}
										else
										{
...
int ListaExpresii(int loc1)
{
	...
	if (Expresie())
	{
		...
		if (cont_par>tabela[loc1].nrpar)
		{
			printf("Eroare, numar de parametrii prea mare! la linia: %d\n",q->linie);
			return 0;
		}
		else
		{
			par=tabela[loc1].listapar;
			i=0;
			while (par!=NULL)
			{
				if (i<=cont_par)
			...
			tipp=t1;//tipul parametrului
			trp=t2;//tipul transmisie
			if (tipe5!= tipp)
			{
				if ((tipe5==3 || tipp==3) || (tipp==2 && trp==2))
				{
					printf("Eroare , parametrul de pe pozitia %d nu corespunde ca tip! la linia: %d\n",cont_par,q->linie);
					return 0;
				}
				if (tipe5==2 && tipp==1) 
				{
			...
			if ((lvale5!=trp) && (trp==2))
			{
				printf("Eroare ,parametrul de pe pozitia %d nu poate fi transmis prin adresa! la linia: %d\n",cont_par,q->linie);
				return 0;
			}
		}

		if (ListaExpresii1(loc1))
			return 1;
		else
		{
...
int ListaExpresii1(int loc1)
{
	...
	if ((q->cod_lexical==4)&&(q->atribut==2))//delimitatorul ,
	{
		...
		if (Expresie())
		{
			...
			if (tipe6!= tipp)
			{
				if ((tipe6==3 || tipp==3) || (tipp==2 && trp==2))
				{
					printf("Eroare , parametrul de pe pozitia %d nu corespunde ca tip! la linia: %d\n",cont_par,q->linie);
					return 0;
				}
		...
		if (Expresie())
		{
			...
			if ((q->cod_lexical==3) &&(q->atribut>=8)&&(q->atribut<=13))//operatorii:<,<=,>,>=,<>,=
			{
				if (q->atribut==8)
					...
					oper1=2;//<=
				else if (q->atribut==10)
					oper1=3;//>
				else if(q->atribut==11)
					oper1=4;//>=
				else if(q->atribut==12)
					oper1=5;//<>
				else if(q->atribut==13)
					oper1=6;//=
				...
				if (Expresie())
				{
					if (tipp!=tipe)
						if ((tipe==3)|| (tipp==3))
						{
							printf("Eroare: tipuri incompatibile la expr relationale la linia: %d",q->linie);
							return 0;
						}
...
int InstrFor()
{
	if ((q->cod_lexical==1)&&(q->atribut==10))//cuvantul cheie for
	{
		...
		if (Variabila())
		{
			...
			if(tipv!=1)
			{
				printf("Eroare: variabila trebuie sa fie intreaga in instr for la linia : %d\n",q->linie);
				return 0;
			}
			...
			if ((q->cod_lexical==3)&&(q->atribut==14))//operatorul :=
			{
				...
				if (Expresie())
				{
					if(tipe!=1)
					{
						printf("Eroare: expresia trebuie sa fie intreaga in instr for la linia : %d",q->linie);
						return 0;
					}
					if (Sens())
					{
...
int ApelProc()
{
	if (q->cod_lexical==0)//identificator
	{
		loc1=Cauta_TS4(q->nume);//caut direct daca e proc sau functie
		if (loc1==-1)
		{
			printf("Eroare, identificatorul nu exista sau identificatorul trebuie sa fie nume de functie sau de procedura la linia: %d\n",q->linie);
			return 0;
		}
	...
	if ((q->cod_lexical==3)&&(q->atribut==4))//operatorul (
	{
		...
		if (ListaExpresii(loc1))
		{
			if ((q->cod_lexical==3)&&(q->atribut==5))//operatorul )
			{
				...
				if (cont_par!=tabela[loc1].nrpar)
				{
					printf("Eroare, nr de parametrii prea mic!!");
					return 0;
				}
